...
class TravelProblem_LocalSearch:
    ...
    def _generate_random_neighbor_fast(
        self, state: List["Landmark"]
    ) -> List["Landmark"]:
        ...
        for _ in range(50):
            ...
            strategy = random.choice(["REPLACE", "SWAP", "ADD", "REMOVE"])

            if strategy == "REPLACE" and len(state_copy) > 0:
                i = random.randint(0, len(state_copy) - 1)
                new_item = random.choice(self.landmarks)
                if new_item.id not in current_ids:
                    state_copy[i] = new_item

            elif strategy == "SWAP" and len(state_copy) > 1:
            ...
            elif strategy == "REMOVE" and len(state_copy) > 1:
                i = random.randint(0, len(state_copy) - 1)
                state_copy.pop(i)

            # The moment we randomly create a valid tweak, return it instantly!
            if self.valid_state(state_copy):
                return state_copy
